...
img_1 = cv2.imread("img_3.jpg", cv2.IMREAD_ANYCOLOR)
img_2 = cv2.imread("img_1.jpg", cv2.IMREAD_ANYCOLOR)
...
if tras_y!=0 and tras_x==0: #traslacion neta horizontal
    if tras_y>0:
        ...
        panoramica[X-tras_x:X-tras_x+aux_pano.shape [0],Y:Y+aux_pano.shape[1],:] = aux_pano.copy() #Ahora el x=0 no está donde estaba, por eso le resto la traslacion y copio normal el aux_pano 
        ...
        panoramica [:,:,:] = 0
            #vuelvo a ponerle los valores antes del resize
        panoramica[X:X+aux_pano.shape [0],Y-tras_y:Y-tras_y+aux_pano.shape[1],:] = aux_pano.copy()
        panoramica[X+tras_x:X+tras_x+img2.shape[0],Y:Y+img2.shape[1],:] = img2.copy()
        aux = (boollayer_1^boollayer)
        plt.imshow(aux)
        plt.show()
...
if tras_x!=0 and tras_y==0: #traslacion neta vertical
    if tras_x>0:
        ...
        panoramica [:,:,:] = 0
            #vuelvo a ponerle los valores antes del resize
        panoramica[X:X+aux_pano.shape [0],Y:Y+aux_pano.shape[1],:] = aux_pano.copy()
        ...
        aux = boollayer.copy() #capaz se puede poner uno solo fuera del if
        if boollayer.shape[0]<(X+x2+img.shape[0]-x1): 
            boollayer.resize (X+x2+img.shape[0]-x1,boollayer.shape[1]) ####
        boollayer[:,:]=False
        boollayer[X+x2-x1:,:] = aux[:,:]
        print (aux.shape[0])
...
# cv2.imwrite('union_tras_hor.jpg', panoramica)
