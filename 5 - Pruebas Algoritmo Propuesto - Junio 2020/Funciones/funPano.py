...
def pano(panoramica1, img, img2, tras_x, tras_y, X, Y, pos1_2): #ver si sacar img y usar solo panoramica
    ...
    if tras_y!=0 and tras_x==0: #traslacion neta horizontal
        if tras_y>0:
            ...
            panoramica [:,:,:] = 0
                #vuelvo a ponerle los valores antes del resize
            if Y+tras_y<0:
                panoramica[X:X+aux_pano.shape [0],-tras_y:-tras_y+aux_pano.shape[1],:] = aux_pano.copy()# no fue checkeado aun
    ...
    if tras_x!=0 and tras_y==0: #traslacion neta vertical
        if tras_x>0:
            ...
            if X+tras_x<0:
                boollayer.resize (X+x2+img2.shape[0]-x1,boollayer.shape[1]) ####
                boollayer[:,:]=False
                boollayer[x2-x1:,:] = aux[:,:]
