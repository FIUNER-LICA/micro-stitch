...
class AppSpinnaker:
    ...
    # Bandera para activar/desactivar el pre-análisis
        ...
        self.new_image = np.zeros((640,480,3),dtype="uint8")
        self.new_image_capture = Event()
        self._image_analisys = False
        if self._image_analisys:
            self.pre_analisys = Thread (target= self.focus_analisys, daemon=True, args=(0,0))
            self.pre_analisys.start()

        # Inicio de variablesm, parámetros y creación de objetos
        self.R = 0 #Fila inicial del frame
        self.C = 0 # Columna inicial del frame
